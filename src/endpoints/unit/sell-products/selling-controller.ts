...
export class SellingController {
    constructor(private readonly saleService: SaleService, private readonly cls: ClsService, private readonly saleProductService: SaleProductService,
        private readonly productService: ProductService, private readonly salePaymentMethodService: SalePaymentMethodService, private readonly paymentMethodService: PaymentMethodService) { }


    @Post('sellProducts')
    async sellProducts(@Body() saleRequest: SaleRequest) {
        const prices = saleRequest.products.map(x => x.sellingPrice * x.productCount)
        ...
        if (total > 0) {
            if (saleRequest.products.map(x => x.productCount).reduce((accumulator, currentValue) => accumulator + currentValue) >= 1) {
                if (saleRequest.products.map(x => x.sellingPrice).reduce((accumulator, currentValue) => accumulator + currentValue) > 0) {
                    if (saleRequest.paymentMethods.map(x => x.amount).reduce((accumulator, currentValue) => accumulator + currentValue) > 0) {
                        if (products.map(x => x.id).includes(saleRequest.products.find(x => x).productId)) {
                            if (paymentMethods.map(x => x.id).includes(saleRequest.paymentMethods.find(x => x).paymentMethodId)) {
                                if (total == saleRequest.paymentMethods.map(x => x.amount).reduce((accumulator, currentValue) => accumulator + currentValue)) {
                                    ...
                                    saleRequest.products.map(product => {
                                        ...
                                        saleProduct.id = uuid(),
                                            saleProduct.price = products.find(x => x.id == product.productId).price,
                                    ...
                                    saleRequest.paymentMethods.map(paymentmehod => {
                                        ...
                                        if (paymentmehod.amount > 0) {
                                            salePaymentMethod.id = uuid(),
                                                salePaymentMethod.paymentMethodId = paymentmehod.paymentMethodId,
                                ...
                                } else throw new error("selling-product.payments-non-valid")
                            }
        ...
        else throw new error("selling-controller.total-not-valid")
    }
}