...
export class SellingController {
  ...
  async sellProducts(@Body() saleRequest: SaleRequest) {
    ...
    const Products$ = this.productService.getByIds(
      saleRequest.products.map(x => x.productId),
    );
    const PaymentMethods$ = this.paymentMethodService.getByIds(
      saleRequest.paymentMethods.map(x => x.paymentMethodId),
    );
    const [productsFromDb, paymentMethodsFromDb] = await Promise.all([
      Products$,
      PaymentMethods$,
    ]);
    ...
    const saleProducts: SaleProduct[] = saleRequest.products.map(product => {
